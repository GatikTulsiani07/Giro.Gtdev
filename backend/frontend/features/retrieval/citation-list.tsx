  ...
  return (
    <article className={cn("relative border-b border-border-subtle last:border-b-0", (open || selected) && "bg-selection before:absolute before:bottom-2 before:left-0 before:top-2 before:w-0.5 before:bg-primary")}>
      <div className="flex min-h-10 items-center gap-2 px-3 py-2">
        <button className="flex min-w-0 flex-1 items-center gap-2 rounded-control text-left focus-ring" onClick={toggle} aria-expanded={open} aria-label={`${index} ${citation.relativeFilePath}, lines ${citation.startLine} to ${citation.endLine}`}>
      ...
      <Drawer open={open && mobile} label={`Citation ${index}: ${citation.relativeFilePath}`} side="full" onClose={() => setOpen(false)}><div className="flex h-[52px] items-center gap-3 border-b border-border-subtle px-4"><div className="min-w-0 flex-1"><p className="truncate type-mono-strong">{citation.relativeFilePath}</p><p className="type-metadata text-muted-foreground">EVIDENCE {index} · L{citation.startLine}–{citation.endLine}</p></div><Button variant="ghost" size="icon-sm" aria-label="Close citation" onClick={() => setOpen(false)}><X className="size-4" /></Button></div><CitationDetail citation={citation} preview={preview} expanded /></Drawer>
    </article>
  );
}
