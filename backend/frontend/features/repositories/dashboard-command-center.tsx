...
export function DashboardCommandCenter({ repositories = [], repositoryCount, repositoriesLoading = false, repositoryError, onRetryRepositories, sessions = [], sessionsLoading = false, sessionError, onRetrySessions }: DashboardCommandCenterProps) {
  ...
  const loading = repositoriesLoading || sessionsLoading;

  return (
    <section aria-label="Engineering command center" aria-busy={loading} className="mt-8">
      {loading ? <p role="status" aria-live="polite" className="sr-only">Loading dashboard repositories and investigations.</p> : null}
      ...
      <SessionTimeline sessions={sessions} loading={sessionsLoading} error={sessionError} onRetry={onRetrySessions} startHref={startHref} announceLoading={false} />
...
function DashboardRepositorySkeleton() {
  return <div aria-hidden="true" className="grid gap-7 laptop:grid-cols-3">{Array.from({ length: 3 }, (_, index) => <div key={index} className="space-y-3 border-y border-border-subtle py-3"><Skeleton className="h-4 w-24" /><Skeleton className="h-20" /><Skeleton className="h-20" /></div>)}</div>;
}
...
function RepositoryActions({ repository, session, loading }: { repository?: IndexedRepository; session?: SessionSummary; loading: boolean }) {
  return <section aria-labelledby="repository-actions-heading" className="mt-9"><div className="mb-4"><h2 id="repository-actions-heading" className="type-section-eyebrow text-muted-foreground">Repository actions</h2><p className="mt-2 type-compact text-text-secondary">Open an existing engineering workflow without changing repository scope.</p></div>{loading ? <div aria-hidden="true" className="grid gap-2 mobile:grid-cols-2 laptop:grid-cols-4"><Skeleton className="h-16" /><Skeleton className="h-16" /><Skeleton className="h-16" /><Skeleton className="h-16" /></div> : <nav aria-label="Repository actions" data-testid="repository-actions-layout" className="grid gap-2 mobile:grid-cols-2 laptop:grid-cols-4">{repository ? <><CommandAction href={repositoryPath(repository)} icon={FolderOpen} title="Open overview" detail={`${repository.owner}/${repository.repo}`} />{repository.status === "indexed" ? <CommandAction href={`${repositoryPath(repository)}/search`} icon={Search} title="Search repository" detail="Inspect ranked evidence" /> : null}{repository.status === "indexing" ? <CommandAction href={`${repositoryPath(repository)}/indexing`} icon={GitBranch} title="View indexing" detail="Inspect backend progress" /> : null}{session ? <CommandAction href={`/chat/${encodeURIComponent(session.id)}`} icon={MessageSquare} title="Continue session" detail={session.title} /> : null}</> : null}<CommandAction href="/repositories/connect" icon={Plus} title="Connect repository" detail="Add another codebase" /></nav>}</section>;
}
